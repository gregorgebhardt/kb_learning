...
def me_mlp(num_me_inputs, dim_me_inputs, me_size=(128, 128), mlp_size=(128, 128), activation=tf.nn.leaky_relu):
    ...
    def network_fn(X):
        ...
        # add layers of the same size as mean embedding to the additional inputs
        with tf.variable_scope('ei'):
            for i, layer_size in enumerate(me_size):
                env_states_input_layer = tf.layers.dense(env_states_input_layer, layer_size, name='fc{}'.format(i + 1),
        ...
        for i, layer_size in enumerate(mlp_size):
            h = tf.layers.dense(h, layer_size, name="fc{}".format(i + 1),
                                kernel_initializer=U.normc_initializer(1.0), activation=activation)
