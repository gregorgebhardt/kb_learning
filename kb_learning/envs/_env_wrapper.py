...
class NormalizeActionWrapper(ActionWrapper):
    def __init__(self, env):
        ...
        self.action_space = Box(low=(self.action_space.low - self.mean) / scale,
                                high=(self.action_space.high - self.mean) / scale,
                                dtype=self.action_space.dtype)
        self.scale_diag = np.diag(scale)
        self.inv_scale_diag = np.diag(1/scale)

    def action(self, action):
        return (action - self.mean).dot(self.scale_diag)

    def reverse_action(self, action):
        return action.dot(self.inv_scale_diag) + self.mean
