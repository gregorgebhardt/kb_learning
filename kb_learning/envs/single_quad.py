...
class QuadPushingEnv(KilobotsEnv):
    ...
    _light_max_dist = .75 * min(world_size)
    ...
    def _configure_environment(self):
        ...
        if _spawn_randomly:
            ...
            _kb_radius = (0.5 * self._object_size + self._light_max_dist * np.random.rand())
            ...
            _light_init = self._object_init[:2] + _kb_radius * np.array([np.cos(angle), np.sin(angle)])
            _light_init = np.minimum(_light_init, self.world_bounds[0] + self._light_radius)
            _light_init = np.maximum(_light_init, self.world_bounds[1] - self._light_radius)
        else:
