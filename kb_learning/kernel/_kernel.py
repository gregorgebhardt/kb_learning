...
#         k_m = np.empty((1, m))
#         k_nm = np.empty((n, m))
#         for i, chunk_n in enumerate(np_chunks(k1_reshaped, num_kb_1)):
#             k_n[i] = self._kernel_func(chunk_n).sum()
#
#             for j, chunk_m in enumerate(np_chunks(k2_reshaped, num_kb_2)):
#                 if i == 0:
#                     # compute the kernel values within each configuration of k2
#                     k_m.flat[j] = self._kernel_func(chunk_m).sum()
#
#                 k_nm[i, j] = self._kernel_func(chunk_n, chunk_m).sum()
#
#         k_n /= num_kb_1 ** 2
#         k_m /= num_kb_2 ** 2
...
class StateKernel:
    ...
    def __init__(self, bandwidth_light=1.0, bandwidth_kb=np.array([1.0]*2), weight=.5, num_processes=1):
